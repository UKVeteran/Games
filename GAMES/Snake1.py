...
class snake():
    def __init__(self,speed, size):
        ...
        self.image.fill((255,255,0)) #SNAKE changes color
        self.speed = speed
        self.size = size
        self.images = []
        self.old_pos = [[20,20]]
    ...
    def add_apple(self):
        ...
        block.fill((0,255,255)) #SNAKE tail changes color
        self.images.append([block,[10,10]])
class apple():
    def __init__(self,size):
        self.pos = [random.randrange(10, 780,10),random.randrange(10,430,10)]
        self.image = pg.Surface((10*size,10*size))
        self.image.fill((255,0,255)) #BLOCK to collect changes color
class game():
    ...
    def over(self):
        while 1:
            ...
            self.screen.blit(txts,txtrect)
            pg.display.update()
            self.make_button((153,300,100,50),'AGAIN', [(255,255,255),(150,150,150)], action = lambda:restart())
            if self.hover ==True:
...
class startmenu():
    ...
    def mainloop(self):
        while 1:
            self.screen.fill((35,38,117))
            self.make_text(400, 150, 'SNAKE', color = (255,255,255), size = 80, a = True)
            for event in pg.event.get():
                if event.type == pg.QUIT:
                    sys.exit()
            for x in self.blocks:
