...
def upClear(x, y):
    ...
    if verticalDoorLeft <= x <= verticalDoorRight and y - 5 < topWall:
        canMove = True
    elif y - 5 < topWall:
        canMove = True
    elif (x < leftWall or x > rightWall) and y - 5 < middleDoorsTop:
        canMove = True
...
def downClear(x, y):
    ...
    if verticalDoorLeft <= x <= verticalDoorRight and bottomWall < y + 5:
        canMove = True
    elif bottomWall < y + 5:
        canMove = True
    elif (x < leftWall or x > rightWall) and y + 5 > middleDoorsBottom:
        canMove = True
...
def leftClear(x, y):
    ...
    if middleDoorsTop <= y <= middleDoorsBottom and x - 5< leftWall:
        canMove = True
    elif x - 1 < leftWall:
        canMove = True
    elif (y > bottomWall or y < topWall) and x - 5 < verticalDoorLeft:
        canMove = True
...
def rightClear(x, y):
    ...
    if middleDoorsTop <= y <= middleDoorsBottom and x + 5 > rightWall:
        canMove = True
    elif x + 1 > rightWall:
        canMove = True
    elif (y > bottomWall or y < topWall) and x + 5 > verticalDoorRight:
        canMove = True
...
def randomPos():
    # return x and y
    x = random.randrange(30, windowSize[0] - 52)
    y = random.randrange(30, windowSize[1] - 52)
    return x, y

def touchingSpark(x, y):
    return -32 < x - sparkPos[0] < 20 and -40 < y - sparkPos[1] < 20
...
spark = pygame.transform.scale2x(spark)
